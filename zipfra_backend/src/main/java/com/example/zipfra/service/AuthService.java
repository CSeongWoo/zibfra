...
public class AuthService {
    ...
    public void signup(SignupRequest request) {
        ...
        if (userMapper.findByEmail(request.getEmail()).isPresent()) {
            throw new ZipfraException(ErrorCode.EMAIL_ALREADY_EXISTS);
        }
    ...
    public TokenDto login(LoginRequest request) {
        User user = userMapper.findByEmail(request.getEmail())
                .orElseThrow(() -> new ZipfraException(ErrorCode.TOKEN_INVALID, HttpStatus.UNAUTHORIZED));

        if (!passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            throw new ZipfraException(ErrorCode.TOKEN_INVALID, HttpStatus.UNAUTHORIZED);
        }
    ...
    public TokenDto refresh(String refreshToken) {
        ...
        try {
            ...
            throw new ZipfraException(ErrorCode.REFRESH_TOKEN_EXPIRED);
        } catch (Exception e) {
