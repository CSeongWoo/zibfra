...
- 거리 감쇠: `t ≤ flat → W=1`, `t > flat → W = 1/(t/flat)²`. **flat = 5분(400m) 기본, 단 교통(transit) 그룹만 3.75분(300m)** (2026-06-25, §11 — "역세권은 더 가까워야" 반영). 아래 W표는 일반(flat=5) 기준.
...
|----|--------|-----|------|-----------|----------|
| MAP-01 | `GET` | `/api/v1/map/markers` | Public | PostGIS / SUMMARY: `ETag`+`max-age=300`+SWR60, DETAIL: `no-store` | `bbox=minLng,minLat,maxLng,maxLat`(4326); `zoom` 1–21 서버 재판정(§7.1); 대각>150km 가드(§7.2); `size` max 200/def 100, `page` 0-based; **검색 필터(DETAIL 한정)** `dealType`·`propertyType`(**다중선택**, 콤마 구분 → `IN`, 원소 enum 검증, 빈=전체)·`priceMin`·`priceMax`(행별 `COALESCE(deal_amount,deposit)` 기준) 전부 optional(§8.1.1); **DETAIL 마커에 4그룹 base 포함**(`transitBase`/`educationBase`/`commerceBase`/`convenienceBase`, 미계산 매물 `0`, §5.1) |
| MAP-03 | `GET` | `/api/v1/map/search` | Public | PostGIS / `no-store` | **단지 검색**(검색 드롭다운, #4); `q`(검색어, 2자 미만=빈 결과), `limit` def 20·max 50(초과 `400 INVALID_PARAM`); `real_estate_sales.building_name ILIKE` → **단지(building_name+lawd_cd) 단위 집계**(대표 좌표 `ST_Centroid`, `dealCount`·면적 min~max·준공년), `dealCount` 내림차순; **지역(시·구·동) 검색은 프론트 카카오 `addressSearch` 담당**(POI 노이즈 배제) |
...
> 줌인(DETAIL, 개별 매물 마커)에 적용한다. **SUMMARY(줌아웃 시·군·구 집계)도 동일 필터를 반영한다**(2026-06-23, §11): 풍선 숫자 = bbox+필터에 매칭되는 `real_estate_sales` **매물 갯수(거래 행 수)**를 실시간 `COUNT`. `region_summary`의 정적 사전집계 거래건수가 아니다(매물 갯수가 필터에 따라 실시간 변동). 모든 필터는 optional·AND 결합, MyBatis 동적 WHERE(`<if>`)로 생략 가능. 필터 부적합 값은 `400 INVALID_PARAM`.
...
- **가격 필터 대상 컬럼**(2026-06-23 변경, §11): `SALE`→`deal_amount`(매매가), `JEONSE`→`deposit`(보증금), **`WOLSE`→`monthly_rent`(월세 기준)**. 매퍼는 행별 `CASE WHEN deal_type='WOLSE' THEN monthly_rent ELSE COALESCE(deal_amount, deposit) END` 으로 적용(혼합 선택 시 각 행 자기 컬럼). 매매엔 보증금 개념 없음. 프론트 가격 입력칸은 **만원 단위**(저장값 그대로, 1억=10000만), 라벨은 선택 거래유형에 따라 매매가/보증금/월세 표기(`priceFilterLabel`).
